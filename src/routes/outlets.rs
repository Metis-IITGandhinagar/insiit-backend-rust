...
async fn get_outlets(State(pool): State<PgPool>) -> Result<JsonResponse<Vec<Outlet>>, (StatusCode, String)> {
...
async fn get_outlet(State(pool): State<PgPool>, Path(id): Path<i32>) -> Result<JsonResponse<Outlet>, (StatusCode, String)> {
    match query_as::<_, Outlet>(
...
async fn add_outlet(State(pool): State<PgPool>, Json(outlet): Json<Outlet>) -> Result<JsonResponse<Outlet>, (StatusCode, String)> {
    match query(
