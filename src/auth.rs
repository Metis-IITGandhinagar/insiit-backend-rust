...
where
    H: Fn(State<AppState>, Request, String) -> F + Clone + Send + Sync + 'static,
    K: IntoResponse,
        ...
        move |State(state): State<AppState>, request: Request| {
            ...
            Box::pin(async move {
                ...
                match permission_clone.granted_to(auth_header.token().to_string(), state_clone.clone()).await {
                    Ok(Some(email)) => handler(State(state_clone), Request::from_parts(parts, body), email).await,
                    Ok(None) => Err((StatusCode::FORBIDDEN, String::from("Forbidden"))),
                    Err(e) => {
